...
impl Actor {
        ...
        RT: Runtime<BS>,
    {
        ...
        rt.transaction(|st: &mut State, rt| {
            ...
            let (_, tx) = ptx
                ...
                .ok_or_else(|| {
                    actor_error!(USR_NOT_FOUND, "no such transaction {:?} to cancel", params.id)
                })?;
            ...
            if !params.proposal_hash.is_empty() && params.proposal_hash != calculated_hash {
                return Err(actor_error!(USR_ILLEGAL_STATE, "hash does not match proposal params"));
            }
        ...
        RT: Runtime<BS>,
    {
        ...
        rt.transaction(|st: &mut State, rt| {
            if !st.is_signer(&resolved_old_signer) {
                return Err(actor_error!(USR_FORBIDDEN, "{} is not a signer", resolved_old_signer));
            }
        ...
        RT: Runtime<BS>,
    {
        ...
        if params.unlock_duration <= 0 {
            return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "unlock duration must be positive"));
        }

        if params.amount.is_negative() {
            return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "amount to lock must be positive"));
        }

        rt.transaction(|st: &mut State, _| {
            if st.unlock_duration != 0 {
                return Err(actor_error!(USR_FORBIDDEN, "modification of unlock disallowed"));
            }
    ...
    RT: Runtime<BS>,
{
    ...
    if threshold_met {
        st.check_available(rt.current_balance(), &txn.value, rt.curr_epoch()).map_err(|e| {
            actor_error!(USR_INSUFFICIENT_FUNDS, "insufficient funds unlocked: {}", e)
        })?;
    ...
    RT: Runtime<BS>,
{
    let txn = ptx
        ...
        .ok_or_else(|| {
            actor_error!(USR_NOT_FOUND, "no such transaction {:?} for approval", txn_id)
        })?;
