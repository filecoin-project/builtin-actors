...
impl Syscalls for MockRuntime {
    ...
    fn verify_seal(&self, seal: &SealVerifyInfo) -> anyhow::Result<()> {
        let exp = self
            ...
            .expect_verify_seal
            .take()
            .ok_or_else(|| actor_error!(USR_ILLEGAL_STATE; "Unexpected syscall to verify seal"))?;

        if exp.seal != *seal {
            return Err(anyhow!(
                actor_error!(USR_ILLEGAL_STATE; "Unexpected seal verification"),
            ));
        }
        if exp.exit_code != ExitCode::OK {
            return Err(anyhow!(ActorError::new(
    ...
    fn verify_post(&self, post: &WindowPoStVerifyInfo) -> anyhow::Result<()> {
        let exp = self
            ...
            .expect_verify_post
            .take()
            .ok_or_else(|| actor_error!(USR_ILLEGAL_STATE; "Unexpected syscall to verify PoSt"))?;

        if exp.post != *post {
            return Err(anyhow!(
                actor_error!(USR_ILLEGAL_STATE; "Unexpected PoSt verification"),
            ));
        }
        if exp.exit_code != ExitCode::OK {
            return Err(anyhow!(ActorError::new(
    ...
    fn verify_consensus_fault(
        ...
        if exp.require_correct_input {
            if exp.block_header_1 != h1 {
                return Err(anyhow!(
                    actor_error!(USR_ILLEGAL_STATE; "Header 1 mismatch")
                ));
            }
            if exp.block_header_2 != h2 {
                return Err(anyhow!(
                    actor_error!(USR_ILLEGAL_STATE; "Header 2 mismatch")
                ));
            }
            if exp.block_header_extra != extra {
                return Err(anyhow!(
                    actor_error!(USR_ILLEGAL_STATE; "Header extra mismatch"),
                ));
            }
        }
        if exp.exit_code != ExitCode::OK {
