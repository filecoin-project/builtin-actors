...
fn mapping_and_nonce_persist_on_outer_call_failure() {
    ...
    let mut pk_a = [0u8; 65]; pk_a[0] = 0x04; for i in 1..65 { pk_a[i] = 0xA1; }
    ...
    let params = evm::ApplyAndCallParams { list: list.clone(), call: evm::ApplyCall { to: b_eth, value: vec![], input: vec![0x01] } };
