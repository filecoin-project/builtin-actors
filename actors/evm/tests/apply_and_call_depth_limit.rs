...
fn apply_and_call_depth_limit_invokeaseoa() {
    ...
    let mut c20 = [0u8; 20];
    c20.copy_from_slice(&keccak_c[12..32]);
    let mut a20 = [0u8; 20];
    ...
    // PUSH20 C address
    bytecode.push(0x73);
    bytecode.extend_from_slice(c_eth.as_ref());
    ...
    let mut initcode: Vec<u8> = vec![0x60, 0x00, 0x60, m, 0x60, n, 0x39, 0x60, 0x00, 0x60, n, 0xF3];
    ...
    // Second ApplyAndCall: set mapping A -> B (B is receiver EVM actor) and call A; should follow A->B and execute B in authority context.
    {
        ...
        let _b_f4 =
            FilAddress::new_delegated(fil_actors_runtime::EAM_ACTOR_ID, &util::CONTRACT_ADDRESS)
                .unwrap();
