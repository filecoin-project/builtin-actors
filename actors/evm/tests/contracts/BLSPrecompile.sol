
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;

contract BLSPrecompileCheck {
    address constant G1_ADD_PRECOMPILE = address(0x0B);

    /// @notice Asserts that G1 addition precompile at 0x0B correctly computes 2·P
    function testG1Add() public view {
        
        // G1 generator coordinates
        uint256 x1 = 0x17F1D3A73197D7942695638C4FA9AC0FC3688C4F9774B905A14E3A3F171BAC58;
        uint256 y1 = 0x08B3F481E3AAA0F1A09E30ED741D8AE4FCF5E095D5D00AF600DB18CB2C04B3ED;

        // Encode input as two G1 points

        bytes memory input = hex"";

        bytes memory EXPECTED_OUTPUT = hex"";
        bytes32 expectedHash = keccak256(EXPECTED_OUTPUT);

        // Call precompile
        (bool success, bytes memory output) = G1_ADD_PRECOMPILE.staticcall(input);
        require(success, "Precompile call failed");
        require(output.length == 128, "Invalid output length");
        bytes32 actualHash = keccak256(output);

        require(actualHash == expectedHash, "Unexpected output");

        uint256 x;
        uint256 y;

        assembly {
            // Skip first 32 bytes of x, load real field value
            x := mload(add(output, 0x40)) // 0x20 + 0x20 = 64
            y := mload(add(output, 0x80)) // 0x20 + 0x20 + 0x20 = 96
        }

        // Expected result of 2·P
        uint256 expectedX = 0x4fda4886854573974fab73b046d3147ba5b7a5bde85279ffede1b45b3918d82d;
        uint256 expectedY = 0x754b07c01ace7834f57f3e7315faefb739e59018e22c492006190fba4a870025;
        require(x == expectedX, "Incorrect X");
        require(y == expectedY, "Incorrect Y");

    }
}



