...
use crate::interpreter::{
    precompiles::{parameter::pad_right, PrecompileError},
    System, U256,
};
...
pub(super) fn modexp<RT: Runtime>(
    ...
    let input = pad_right(input, base_len + exponent_len + mod_len);
...
mod tests {
    ...
    fn padding() {
        ...
        for i in 12..64 {
            ...
            let res = pad_right(&input, i);
            assert_eq!(&*res, &expected);

            input.push(0);
        }
        let no_padding = b"foo bar boxy                     ".to_vec();
        let res = pad_right(&no_padding, 12);
        // do nothing
