...
impl Actor {
    ...
    fn resolve_account<RT>(rt: &mut RT, raw: &Address) -> Result<Address, ActorError>
    where
        RT: Runtime,
    {
    ...
    pub fn update_channel_state<RT>(
        rt: &mut RT,
        ...
        RT: Runtime,
    {
        ...
        if let Some(extra) = &sv.extra {
            rt.send(&extra.actor, extra.method, extra.data.clone(), TokenAmount::from_atto(0u8))
        ...
        rt.transaction(|st: &mut State, rt| {
            ...
            if new_send_balance < TokenAmount::from_atto(0) {
                return Err(actor_error!(illegal_argument;
    ...
    pub fn settle<RT>(rt: &mut RT) -> Result<(), ActorError>
    where
        RT: Runtime,
    {
        rt.transaction(|st: &mut State, rt| {
    ...
    pub fn collect<RT>(rt: &mut RT) -> Result<(), ActorError>
    where
        RT: Runtime,
    {
        let st: State = rt.state()?;
