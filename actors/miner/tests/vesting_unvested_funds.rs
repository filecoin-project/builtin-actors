...
fn unlock_unvested_funds_leaving_bucket_with_non_zero_tokens() {
    ...
    let amount_unlocked =
        h.unlock_unvested_funds(vest_start, &TokenAmount::from_atto(39)).unwrap().0;
...
fn unlock_unvested_funds_leaving_bucket_with_zero_tokens() {
    ...
    let amount_unlocked =
        h.unlock_unvested_funds(vest_start, &TokenAmount::from_atto(40)).unwrap().0;
...
fn unlock_all_unvested_funds() {
    ...
    h.add_locked_funds(vest_start, &vest_sum, &vspec).unwrap();
    let unvested_funds = h.unlock_unvested_funds(vest_start, &vest_sum).unwrap().0;
...
fn unlock_unvested_funds_value_greater_than_locked_funds() {
    ...
    h.add_locked_funds(vest_start, &vest_sum, &vspec).unwrap();
    let unvested_funds =
        h.unlock_unvested_funds(vest_start, &TokenAmount::from_atto(200)).unwrap().0;
    assert_eq!(vest_sum, unvested_funds);
...
fn unlock_unvested_funds_when_there_are_vested_funds_in_the_table() {
    ...
    let (unvested_funds, total_unlocked) = h.unlock_unvested_funds(new_epoch, &target).unwrap();
    ...
    assert_eq!(
        &[60][..],
        &h.st.vesting_funds.load(&h.store).unwrap().map(|vf| vf.epoch).collect::<Vec<_>>()
    );
}
