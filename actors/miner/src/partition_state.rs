...
impl Partition {
    ...
    pub fn reschedule_expirations<BS: Blockstore>(
        ...
        let sector_numbers = sector_numbers.validate().map_err(|e| {
            actor_error!(USR_ILLEGAL_ARGUMENT, "failed to validate rescheduled sectors: {}", e)
        })?;
    ...
    pub fn terminate_sectors<BS: Blockstore>(
        ...
        let sector_numbers = sector_numbers.validate().map_err(|e| {
            actor_error!(USR_ILLEGAL_ARGUMENT, "failed to validate terminating sectors: {}", e)
        })?;
    ...
    pub fn record_skipped_faults<BS: Blockstore>(
        ...
        let skipped = skipped.validate().map_err(|e| {
            actor_error!(USR_ILLEGAL_ARGUMENT, "failed to validate skipped sectors: {}", e)
        })?;
