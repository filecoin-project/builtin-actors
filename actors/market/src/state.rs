...
use crate::{deal_id_key, sector_number_key};
use cid::Cid;
...
impl State {
    ...
    pub fn load_pending_deal_allocation_ids<'bs, BS>(
        &mut self,
        store: &'bs BS,
    ) -> Result<Map<'bs, BS, AllocationID>, ActorError>
    where
        BS: Blockstore,
    {
        make_map_with_root_and_bitwidth(&self.pending_deal_allocation_ids, store, HAMT_BIT_WIDTH)
            .context_code(
                ExitCode::USR_ILLEGAL_STATE,
        ...
        BS: Blockstore,
    {
        self.pending_deal_allocation_ids = pending_deal_allocation_ids.flush().context_code(
            ExitCode::USR_ILLEGAL_STATE,
            "failed to flush pending deal allocation id's",
        )?;
        Ok(())
    }
        ...
        BS: Blockstore,
    {
        let mut pending_deal_allocation_ids = self.load_pending_deal_allocation_ids(store)?;
        let maybe_alloc_id =
            remove_pending_deal_allocation_id(&mut pending_deal_allocation_ids, deal_id)?;
        self.save_pending_deal_allocation_ids(&mut pending_deal_allocation_ids)?;
        Ok(maybe_alloc_id)
    }
...
pub fn remove_pending_deal_allocation_id<BS>(
    pending: &mut Map<BS, AllocationID>,
    deal: DealID,
) -> Result<Option<AllocationID>, ActorError>
where
    BS: Blockstore,
{
    Ok(pending
        .delete(&deal_id_key(deal))
        .with_context_code(ExitCode::USR_ILLEGAL_STATE, || {
            format!("failed to remove pending allocation ID for {}", deal)
        })?
        .map(|(_k, v)| v))
}
