...
impl Actor {
        ...
        RT: Runtime<BS>,
    {
        if params.amount < TokenAmount::from(0) {
            return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "negative amount: {}", params.amount));
        }
        ...
        RT: Runtime<BS>,
    {
        ...
        let provider = rt.resolve_address(&provider_raw).ok_or_else(|| {
            actor_error!(USR_NOT_FOUND, "failed to resolve provider address {}", provider_raw)
        })?;
        ...
        for (di, mut deal) in params.deals.into_iter().enumerate() {
            ...
            *lockup += deal.proposal.client_balance_requirement();
            ...
            let pcid = deal.proposal.cid().map_err(|e| {
                actor_error!(USR_ILLEGAL_ARGUMENT; "failed to take cid of proposal {}: {}", di, e)
            })?;
        ...
        if valid_deal_count == 0 {
            return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "All deal proposals invalid"));
        }
    ...
    RT: Runtime<BS>,
{
    ...
    proposal
        .piece_size
        .validate()
        .map_err(|e| actor_error!(USR_ILLEGAL_ARGUMENT, "proposal piece size is invalid: {}", e))?;

    // * we are skipping the check for if Cid is defined, but this shouldn't be possible

    if !is_piece_cid(&proposal.piece_cid) {
        return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "proposal PieceCID undefined"));
    }

    if proposal.end_epoch <= proposal.start_epoch {
        return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "proposal end before proposal start"));
    }

    if rt.curr_epoch() > proposal.start_epoch {
        return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "Deal start epoch has already elapsed."));
    };

    let (min_dur, max_dur) = deal_duration_bounds(proposal.piece_size);
    if proposal.duration() < min_dur || proposal.duration() > max_dur {
        return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "Deal duration out of bounds."));
    };
    ...
    {
        return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "Storage price out of bounds."));
    };
    ...
    {
        return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "Provider collateral out of bounds."));
    };
    ...
    {
        return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "Client collateral out of bounds."));
    };
