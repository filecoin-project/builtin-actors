...
use fil_actor_multisig::{ProposeParams, Method as MsigMethod, Transaction, compute_proposal_hash, TxnIDParams, TxnID};
...
use test_vm::util::{create_accounts, apply_ok, apply_code};
...
fn test_proposal_hash() {
    ...
    let wrong_approval_params = TxnIDParams{
        id: TxnID(0),
        proposal_hash: wrong_hash.to_vec(),        
    };
    apply_code(&v, bob, msig_addr, TokenAmount::zero(), MsigMethod::Approve as u64, wrong_approval_params, ExitCode::USR_ILLEGAL_ARGUMENT);
    ...
    let approve_params = TxnIDParams{
        id: TxnID(0),
        proposal_hash: correct_hash.to_vec(),
    };
