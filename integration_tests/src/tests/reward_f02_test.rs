...
use fil_actor_reward::{
    AwardBlockRewardParams, CancelPendingParams, DENOM, DistributionInit, ExplicitDistribution,
    Method as RewardMethod, PendingWrite, PendingWriteOp, RecipientShare, RegisterStreamParams,
    RegisterStreamPayload, RemoveStreamParams, SetDistributionParams, SetDistributionPayload,
    SetWeightRecordsParams, State as RewardState, Stream, StreamsState, WeightRecord,
    WeightRecordUpdate,
};
...
fn award(v: &dyn VM, miner: Address) {
    apply_ok_implicit(
        v,
        &SYSTEM_ACTOR_ADDR,
        &REWARD_ACTOR_ADDR,
...
#[vm_test]
pub fn reward_f02_award_pays_recipients(v: &dyn VM) {
    ...
    // A 4 atto block reward at 25% each: one atto to the miner, one to the service recipient,
    // and the two unallocated attos burnt.
    ...
    assert_eq!(
        &reward_before - &miner_reward - &service_reward - &burn,
        v.balance(&REWARD_ACTOR_ADDR)
    );
    ...
    // A pending removal changes nothing before its effective epoch, so this award still pays.
    v.set_epoch(1);
    mutate_state(v, &REWARD_ACTOR_ADDR, |state: &mut RewardState| state.epoch = 1);
    let recipient_before = v.balance(&recipient);
    award(v, miner);
    assert_eq!(recipient_before + &service_reward, v.balance(&recipient));
    assert!(take_last_events(v).is_empty());
    let (state, streams) = load_reward_state(v);
    assert_eq!(vec![removal.clone()], streams.pending_writes);
    ...
    // The removal applies with this award, so the service recipient is paid nothing more and the
    // stream's share of the reward burns.
    let recipient_before = v.balance(&recipient);
    let burn_before = v.balance(&BURNT_FUNDS_ACTOR_ADDR);
    award(v, miner);
    assert_eq!(recipient_before, v.balance(&recipient));
    assert_eq!(burn_before + TokenAmount::from_atto(3), v.balance(&BURNT_FUNDS_ACTOR_ADDR));
    assert_eq!(vec![write_event("write-applied", &removal, false)], take_last_events(v));

    let (state, streams) = load_reward_state(v);
    assert_eq!(vec![CONSENSUS_STREAM_ID], streams.streams.iter().map(|s| s.id).collect::<Vec<_>>());
    assert_eq!(TokenAmount::from_atto(12), state.total_minted_reward);
    assert_eq!(TokenAmount::from_atto(7), state.total_burn_minted);
    assert_eq!(TokenAmount::from_atto(2), state.total_explicit_minted);
    assert_invariants(v, &Policy::default(), None);
}
...
pub fn reward_f02_queued_apply_and_drop(v: &dyn VM) {
    ...
    v.set_epoch(2);
    mutate_state(v, &REWARD_ACTOR_ADDR, |state: &mut RewardState| state.epoch = 2);
    // Cancelling an empty slot does nothing of its own, so the due writes are all that move.
    apply_ok(
        v,
        &swa,
        &REWARD_ACTOR_ADDR,
        &TokenAmount::zero(),
        RewardMethod::CancelPendingExported as u64,
        Some(CancelPendingParams { id: Some(999), op: PendingWriteOp::RemoveStream }),
    );
    assert_eq!(
        vec![
